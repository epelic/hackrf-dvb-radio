...
try {
    ...
    & $Python -m PyInstaller --noconfirm --clean --windowed --name HackRFDVBRadio --icon "assets\app.ico" --add-data "dvb_tx.py;." --add-data "assets\app.ico;assets" --add-data "assets\app.png;assets" --add-binary "$RadioBin\tcl86t.dll;." --add-binary "$RadioBin\tk86t.dll;." --add-binary "$RadioBin\libcrypto-3-x64.dll;." --add-binary "$RadioBin\libssl-3-x64.dll;." --add-binary "$RadioBin\liblzma.dll;." --add-binary "$RadioBin\libbz2.dll;." --add-binary "$RadioBin\libexpat.dll;." app.py
